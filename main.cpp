...
bool split(const std::string &i_name, std::vector<std::string> &o_names,
  ...
  size_t chunk_size = mem_records_num * sizeof(T);
  ...
  while (!done) {
    ...
    std::sort(mem.begin(), mem.begin() + records_read);
    std::string o_name = create_filename("tmp_split_file_", o_file_counter++);
    std::ofstream out(o_name.c_str(), std::ios::out | std::ios::binary);
    out.write((char *)&mem[0], records_read * sizeof(T));
    if (!out) {
